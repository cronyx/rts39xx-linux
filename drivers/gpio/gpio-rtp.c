...
static int rtp_gpio_get(struct gpio_chip *gc, unsigned int offset)
{
	uint8_t val;
	struct rtp_gpio *rtp_gpio = gpiochip_get_data(gc);
	struct rtp_mfd_chip *chip = rtp_gpio->chip;

	dev_dbg(NULL, "Get rtp gpio-%d value\n", offset);
	switch (offset) {
	case 0:
	case 1:
	...
	case 5:
		...
		if (val == 0x01)
			...
			return 0;
	default:
		dev_warn(NULL, "rtp gpio 0-5, offset = %d\n", offset);
		return -EINVAL;
	}
...
static void rtp_gpio_set(struct gpio_chip *gc, unsigned int offset,
			int value)
{
	uint8_t val;
	struct rtp_gpio *rtp_gpio = gpiochip_get_data(gc);
	...
	dev_dbg(NULL, "Set rtp gpio-%d value, value = %d\n", offset, value);
	if (value) {
		switch (offset) {
		case 0:
		case 1:
...
static int rtp_gpio_doutput(struct gpio_chip *gc, unsigned int offset,
				int value)
{
	struct rtp_gpio *rtp_gpio = gpiochip_get_data(gc);
	struct rtp_mfd_chip *chip = rtp_gpio->chip;

	dev_dbg(NULL, "Set rtp gpio-%d as output, value %d\n",
				offset, value);
	if (value) {
		switch (offset) {
		case 0:
...
static int rtp_gpio_dinput(struct gpio_chip *gc, unsigned int offset)
{
	struct rtp_gpio *rtp_gpio = gpiochip_get_data(gc);
	struct rtp_mfd_chip *chip = rtp_gpio->chip;

	dev_dbg(NULL, "Set rtp gpio-%d as input\n", offset);
	switch (offset) {
	case 0:
	case 1:
		return rtp_update_bits(chip,
	...
	case 4:
	case 5:
		dev_warn(NULL, "rtp gpio-3/4/5 only output\n");
		return 0;
	default:
		dev_warn(NULL, "rtp gpio 0-5, offset = %d\n", offset);
		return -EINVAL;
					    ...
					    const char *buf, size_t count)
{
	...
	dev_dbg(NULL, "Set rtp gpio-%d direction, dir = %d\n",
				gpio_idx, gpio_dir);
...
static int rtp_gpio_probe(struct platform_device *pdev)
{
	...
	struct rtp_board *pmic_plat_data = chip->rtp_data;
	...
	rtp_gpio->gpio_chip.parent = &pdev->dev;
	...
	ret = gpiochip_add_data(&rtp_gpio->gpio_chip, rtp_gpio);
	if (ret) {
...
sysfs_failed:
	...
	gpiochip_remove(&rtp_gpio->gpio_chip);
