...
static void rtscam_produce_desc(void)
{
	...
	uvc_input_header.wTotalLength =
		streamdesclen + uvc_input_header.bLength;
	uvc_input_header.bNumFormats = count;

}

static int rtscam_query_desc(int fmtindex,
	int frmindex, struct format_setting *fts)
{
	struct format_setting_list *q = 0;
	int i = 1;
			      ...
			      unsigned int cmd, unsigned long arg)
{
	...
	case ANDROIDUVC_IOC_PRODUCE_DESC:
		...
		rtscam_produce_desc();
		usb_add_config(uvccdev, uvcconf, rtsx_uvc_config_bind);
#endif
		break;
...
{
	rlxprintk(RLX_TRACE_DEBUG, "uvc function enable\n");
