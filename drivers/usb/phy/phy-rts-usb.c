...
#define UPHY_DEV_SRC_MASK		0x07
	...
	u32 aphy_cfg5;
#define UPHY_ANA_AUTO_K			(1 << 27)
#define UPHY_Z0_CODE_SHIFT		28
...
void rts_usb_clear_phy_int(struct usb_phy *phy,
...
/*
	if (RTS_SOC_HW_ID.hw_ver == HW_ID_VER_RTS3901) {
		if (RTS_SOC_HW_ID.hw_rev == HW_ID_REV_C) {
		...
		cfg &= ~(UPHY_HST_SEN_HST_MASK << UPHY_HST_SEN_HST_SHIFT);
		cfg |= (RTS3901_HOST_DISCONN_THD << UPHY_HST_SEN_HST_SHIFT);
...
*/
	...
	/* Disable device RX calibration and change output slew rate setting */
	cfg = readl(&rts->regs->aphy_cfg4);
	cfg &= ~(UPHY_DEV_CAL | UPHY_DEV_SRC_MASK);
	cfg |= RTS_SRC_VAL;
	writel(cfg, &rts->regs->aphy_cfg4);

	/* Device sensitivity */
	cfg = readl(&rts->regs->aphy_cfg3);
	cfg &= ~(UPHY_DEV_SEN_MASK << UPHY_DEV_SEN_SHIFT);
	cfg |= (RTS_DEV_SEN << UPHY_DEV_SEN_SHIFT);
	writel(cfg, &rts->regs->aphy_cfg3);

	/* Disable host RX calibration and change output slew rate setting */
	cfg = readl(&rts->regs->aphy_cfg2);
	cfg &= ~(UPHY_HST_CAL | UPHY_HST_SRC_MASK);
	cfg |= RTS_SRC_VAL;
	writel(cfg, &rts->regs->aphy_cfg2);

	/* Host sensitivity */
	cfg = readl(&rts->regs->aphy_cfg1);
	cfg &= ~(UPHY_HST_SEN_MASK << UPHY_HST_SEN_SHIFT);
	cfg |= (RTS_HST_SEN << UPHY_HST_SEN_SHIFT);
	writel(cfg, &rts->regs->aphy_cfg1);
...
static struct pinctrl *usbphy_pinctrl;
...
static int rts_usb_phy_probe(struct platform_device *pdev)
{
	...
	usbphy_pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(usbphy_pinctrl))
		return PTR_ERR(usbphy_pinctrl);

	default_state = pinctrl_lookup_state(usbphy_pinctrl,
					     PINCTRL_STATE_DEFAULT);
	if (IS_ERR(default_state)) {
		dev_warn(&pdev->dev, "have not set default pinctrl state\n");
	} else {
		ret = pinctrl_select_state(usbphy_pinctrl, default_state);
		if (ret) {
...
{
	struct pinctrl_state *sleep_state = NULL;
	int ret = 0;
	...
	sleep_state = pinctrl_lookup_state(usbphy_pinctrl, PINCTRL_STATE_SLEEP);
	...
	ret = pinctrl_select_state(usbphy_pinctrl, sleep_state);
	if (ret)
