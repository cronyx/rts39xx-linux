...
static void rtl8168_mac_loopback_test(struct rtl8168_private *tp)
{
    ...
    mapping = dma_map_page(&tp->platform_dev->dev, virt_to_page(skb->data),
...
//              dev_printk(KERN_INFO, &tp->platform_dev->dev, "loopback test finished\n",rx_len,len);
    ...
    tp->cur_tx++;
    tp->cur_rx++;
    dma_unmap_page(&tp->platform_dev->dev, le64_to_cpu(mapping),
                     len, DMA_TO_DEVICE);
...
u16 mmd_read(struct rtl8168_private *tp, u16 devad, u16 addr)
{
	...
	reg13dat = mdio_read(tp, 0x0D);
	reg13dat &= 0x3FFF;
	reg13dat = (reg13dat & 0xFFE0) | devad;
	mdio_write(tp, 0x0D, reg13dat);
	mdio_write(tp, 0x0E, addr);
	reg13dat = (reg13dat & 0x3FFF) | 0x4000;
	mdio_write(tp, 0x0D, reg13dat);
	date = mdio_read(tp, 0x0E);
                   ...
                   struct resource **irq_out)
{
    ...
    struct clk *clk;
    struct reset_control *eth_reset, *eth_mem_up;
	...
	if (IS_ERR(eth_reset)) {
		...
		return PTR_ERR(eth_reset);
	}

	/* Assert eth-mem-up */
	reset_control_assert(eth_mem_up);
	...
	if (rc) {
		dev_err(&pdev->dev, "deassert eth-reset fail\n");
		rc = -6;
		goto err_out_res;
	}
	...
	if (rc) {
		dev_err(&pdev->dev, "assert eth_reset fail\n");
		rc = -7;
		goto err_out_res;
	}
	msleep(20);
...
static void __devinit rtl8168_init_led(struct rtl8168_private *tp)
{
	...
	leds = of_find_node_by_path("/ocp/rtl8168@18400000/led_configs");
	if (!leds) {
		dev_info(&pdev->dev, "failed to get ethernet dt data, skip\n");
		return;
	}

	for_each_child_of_node(leds, led) {
		u32 val = 3;
		u32 bank = 0;

		of_property_read_u32(led, "led-pin", &val);
		if (val > 2 || val < 0)
			continue;

		val = 0;
		of_property_read_u32(led, "link-mode", &val);
		switch(val) {
		case 10:
			bank |= LINK10M;
			break;
		...
		val = 0;
		of_property_read_u32(led, "act-full", &val);
		if (val)
			bank |= ACT_FULL;
		reg |= bank << (i * 4);

		val = 0;
		of_property_read_u32(led, "act-high-active", &val);
		if (val)
			reg |= BIT(12 + i);
                    ...
                    struct RxDesc *desc)
{
    ...
    dma_unmap_page(&tp->platform_dev->dev, le64_to_cpu(desc->addr), tp->rx_buf_sz,
                     DMA_FROM_DEVICE);
                     ...
                     int rx_buf_sz)
{
    ...
    *sk_buff = skb;
    ...
    mapping = dma_map_page(&pdev->dev, virt_to_page(skb->data),
		    (size_t) skb->data & ~PAGE_MASK,
                     ...
                     struct TxDesc *desc)
{
    ...
    dma_unmap_page(&pdev->dev, le64_to_cpu(desc->addr), len, DMA_TO_DEVICE);
    desc->opts1 = 0x00;
    desc->opts2 = 0x00;
    desc->addr = 0x00;
    tx_skb->len = 0;
                   ...
                   u32 opts2)
{
    ...
    for (cur_frag = 0; cur_frag < info->nr_frags; cur_frag++) {
        ...
        mapping = skb_frag_dma_map(&tp->platform_dev->dev, frag, 0, len, DMA_TO_DEVICE);
    ...
    for (; rx_left > 0; rx_left--, cur_rx++) {
       ...
       /*
            ...
            if (rtl8168_try_rx_copy(&skb, pkt_size, desc,
                                    tp->rx_buf_sz)) {
                dma_unmap_page(&tp->platform_dev->dev, le64_to_cpu(desc->addr),
...
static const struct of_device_id rtl8168_dt_ids[] = {
	 ...
	 { .compatible = "realtek,rts3901-r8168", },
	 { .compatible = "realtek,rle0745-r8168", },
	 { /* sentinel */ },
};
