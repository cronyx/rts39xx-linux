...
static void *hconf_cache_expand(struct rts_hconf *hconf, int num)
{
	...
	if (!header->cap_num) {
		...
		entry = kcalloc(header->cap_num, sizeof(*entry), GFP_KERNEL);
	}
...
static int __init hconf_init(void)
{
	...
	hconf = kcalloc(HCONF_CHAR_NUM, sizeof(*hconf), GFP_KERNEL);
	if (!hconf) {
		err = -ENOMEM;
		goto unregister;
	}
