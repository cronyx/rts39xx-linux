...
static void gic_handle_shared_int(bool chained)
{
	...
	for (i = 0; i < BITS_TO_LONGS(numintr); i++) {
		pending[i] = GIC_REG32p(PEND, i*4);
		intrmask[i] = GIC_REG32p(MASK, i*4);
	}

	bitmap_and(pending, pending, intrmask, numintr);
	bitmap_and(pending, pending, pcpu_mask, numintr);

	intr = find_first_bit(pending, numintr);
	while (intr != numintr) {
				...
				unsigned int nr_irqs, void *arg)
{
	...
	if (spec->type == GIC_DEVICE) {
		...
		base_hwirq = find_first_bit(ipi_resrv, numintr);
		if (base_hwirq == numintr) {
			return -ENOMEM;
		}
			      ...
			      struct device_node *node)
{
	...
	gic_irq_domain = irq_domain_add_simple(node, numintr, irqbase,
