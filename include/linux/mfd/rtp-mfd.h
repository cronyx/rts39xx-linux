...
/* trim register */
#define RTP_REG_LDO3_TRIM		(0x69)
#define RTP_REG_LDO5_TRIM		(0x6B)

#define RTP_REG_LDO3_TRIM_OFFSET	(0xB8)
#define RTP_REG_LDO5_TRIM_OFFSET	(0xBA)
#define RTP_REG_DC2_TRIM_OFFSET		(0xB9)
#define RTP_REG_DC3_TRIM_OFFSET		(0xBF)
#define RTP_REG_DC4_TRIM_OFFSET		(0xB8)
...
/* trim info */
#define RTP_TRIM_DC4_OFFSET_BITS	(2)
#define RTP_TRIM_DC4_OFFSET_SHIFT	(3)
...
#define RTP_LDO(_pmic, _id, min, max, step, vreg, shift, nbits, ereg,	\
		ebit, svreg, sereg, voffset)				\
{									\
	.desc = {							\
		.name		= (_pmic),				\
...
#define RTP_BUCK(_pmic, _id, min, max, step, vreg, shift, nbits, ereg,	\
			ebit, svreg, sereg, voffset)			\
{									\
	.desc	= {							\
		.name		= (_pmic),				\
...
struct rtp_board {
	...
	struct device_node *rtp_pmic_of_node[RTP_ID_REGULATORS_NUM];
	int regulator_vol[RTP_ID_REGULATORS_NUM];
};
...
struct rtp_mfd_chip {
	...
	/* regulator */
	struct rtp_pmu_info pmu_info[RTP_ID_REGULATORS_NUM];
	struct rtp_trim_info *trim_info;
...
int rtp_bulk_read(struct rtp_mfd_chip *chip, uint8_t reg,
		int count, uint8_t *val);
int rtp_bulk_write(struct rtp_mfd_chip *chip, uint8_t reg,
		int count, uint8_t *val);
